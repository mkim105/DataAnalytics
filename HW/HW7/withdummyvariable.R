...
summary(m1) # 1. hp(p-value = 0.5291 > 0.05) 제거
m1 = glm(logmpg~displacement+wt+acc)
summary(m1) # 2. acc(p-value = 0.0552 > 0.05) 제거
m1 = glm(logmpg~displacement+wt) # 최종 모델  
...
m2 = step(m2, direction = "backward", trace=TRUE) # 최종 모델, AIC가 작을수록 좋음 AIC:-322.17
