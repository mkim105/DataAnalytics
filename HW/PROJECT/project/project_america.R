...
### 지역별로 데이터 만들기 ###
ameridata = newdata[newdata$CountryAmericas==1,]
ameridata = ameridata[c(-1,-4,-3,-5)]
colnames(ameridata)
...
cAm = ameridata$CountryAmericas
sts = ameridata$Status
lfey = ameridata$Life.expectancy
atmy = ameridata$Adult.Mortality
achl = ameridata$Alcohol
hb = ameridata$Hepatitis.B
bmi = ameridata$BMI
ufdth = ameridata$under.five.deaths
plio = ameridata$Polio
dpria = ameridata$Diphtheria
hiv = ameridata$HIV.AIDS
gdp = ameridata$GDP
th119 = ameridata$thinness..1.19.years
sch = ameridata$Schooling

full = glm(lfey~cAm+sts+atmy+achl+hb+bmi+ufdth+plio+dpria+hiv+gdp+th119+sch)
base = glm(lfey~achl)

# model 1  Both direction by stepwise()
m1 = step(base, scope=list(upper=full, lower=~1), direction="both", trace=T)
summary(m1)  # AIC 2579.8


# m2 Forward Selection by step()
m2 = step(base, scope=list(upper=full, lower=~1), direction="forward", trace=T) 
summary(m2) # AIC: 2579.8

# m3 Backward Elimination by step() based on AIC 
m3 = step(full, direction = "backward", trace=TRUE) 
summary(m3) # AIC: 2579.8
...
library(leaps)
regsubsets.out <-
  regsubsets(lfey~ cAm + sts + atmy + achl + hb + bmi + ufdth + 
...
m4 = glm(lfey ~ sts + atmy + achl + hb + ufdth + 
           plio + dpria + hiv + gdp + th119 + sch)
summary(m4) # AIC: 2581.3
...
m1 = glm(formula = lfey ~ achl + th119 + sch + atmy + hiv + ufdth + 
           gdp)
...
library(boot)
mse1 = cv.glm(ameridata, m1, K=10)$delta
mse1

# Removing Outliers
cooksd <- cooks.distance(m1)
influential <- as.numeric(names(cooksd)[(cooksd > (4/528))])
influential
ameridata <- ameridata[-influential, ]
...
m1 = glm(formula = lfey ~ achl + th119 + sch + atmy + hiv + ufdth + 
           gdp)
mse1_new = cv.glm(ameridata, m1, K=10)$delta
